...
import com.vzan.geetionlib.utils.ViewUtils;
...
public class SwipeRefreshLayout extends ViewGroup implements AutoScroller.ScrollerCallback {
    ...
    private int mLoadMoreCompleteToDefaultScrollingDuration =
            DEFAULT_LOAD_MORE_COMPLETE_DELAY_DURATION;
    ...
    public SwipeRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RefreshLayout,
                defStyleAttr, 0);
        try {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (attr == R.styleable.RefreshLayout_refresh_enabled) {
                    ...
                    setLoadMoreTriggerOffset(a.getDimensionPixelOffset(attr, 0));
                        ...
                        .RefreshLayout_swiping_to_refresh_to_default_scrolling_duration) {
                    setSwipingToRefreshToDefaultScrollingDuration(a.getInt(attr,
                            DEFAULT_SWIPING_TO_REFRESH_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable
                        .RefreshLayout_release_to_refreshing_scrolling_duration) {
                    ...
                    setRefreshCompleteDelayDuration(a.getInt(attr,
                            DEFAULT_REFRESH_COMPLETE_DELAY_DURATION));

                } else if (attr == R.styleable
                        .RefreshLayout_refresh_complete_to_default_scrolling_duration) {
                    setRefreshCompleteToDefaultScrollingDuration(a.getInt(attr,
                            DEFAULT_REFRESH_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable
                        .RefreshLayout_default_to_refreshing_scrolling_duration) {
                    setDefaultToRefreshingScrollingDuration(a.getInt(attr,
                            DEFAULT_DEFAULT_TO_REFRESHING_SCROLLING_DURATION));

                } else if (attr == R.styleable
                        .RefreshLayout_swiping_to_load_more_to_default_scrolling_duration) {
                    setSwipingToLoadMoreToDefaultScrollingDuration(a.getInt(attr,
                            DEFAULT_SWIPING_TO_LOAD_MORE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable
                        .RefreshLayout_release_to_loading_more_scrolling_duration) {
                    ...
                    setLoadMoreCompleteDelayDuration(a.getInt(attr,
                            DEFAULT_LOAD_MORE_COMPLETE_DELAY_DURATION));

                } else if (attr == R.styleable
                        .RefreshLayout_load_more_complete_to_default_scrolling_duration) {
                    setLoadMoreCompleteToDefaultScrollingDuration(a.getInt(attr,
                            DEFAULT_LOAD_MORE_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable
                        .RefreshLayout_default_to_loading_more_scrolling_duration) {
    ...
    private void layoutChildren() {
        ...
        if (mFooterView != null) {
            ...
            switch (mStyle) {
                case STYLE.CLASSIC:
                    bottom = height - paddingBottom - lp.bottomMargin + mFooterHeight +
                            mFooterOffset;
                    break;
                case STYLE.ABOVE:
                    bottom = height - paddingBottom - lp.bottomMargin + mFooterHeight +
                ...
                case STYLE.BLOW:
                    ...
                    break;
                case STYLE.SCALE:
                    bottom = height - paddingBottom - lp.bottomMargin + mFooterHeight / 2 +
                            mFooterOffset / 2;
                    break;
                default:
                    // classic
    ...
    protected void onFinishInflate() {
        ...
        if (childNum == 0) {
            ...
            mFooterView = findViewById(R.id.swipe_loadmore_footer);
        } else {
    ...
    protected ViewGroup.LayoutParams generateDefaultLayoutParams() {
        return new SwipeRefreshLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                /**
                 * 在SWIPING_TO_REFRESH、SWIPING_TO_LOAD_MORE、RELEASE_TO_REFRESH
                 * 和RELEASE_TO_LOAD_MORE四种状态释放后，
                 * 手指再次按下，需停止滚动
                 */
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //是否竖直滑动
                boolean isMove = Math.abs(deltaX) < Math.abs(deltaY) && Math.abs(deltaY) >
                        mTouchSlop;
                //是否能下拉刷新
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                //注意：此处徐返回true，否则在onInterceptTouchEvent中处理的功能失效
                // (在SWIPING_TO_REFRESH、SWIPING_TO_LOAD_MORE、
    ...
    private void fingerScroll(final float yDiff) {
        ...
        if ((mTargetOffset > 0 && tmpTargetOffset < 0) || (mTargetOffset < 0 && tmpTargetOffset >
                0)) {
         ...
         */
        if (mRefreshFinalDragOffset > mRefreshTriggerOffset && tmpTargetOffset >
                mRefreshFinalDragOffset) {
            yScroll = mRefreshFinalDragOffset - mTargetOffset;
        } else if (mLoadMoreFinalDragOffset > mLoadMoreTriggerOffset && -tmpTargetOffset >
                mLoadMoreTriggerOffset) {
    ...
    private boolean onCheckCanRefresh() {
        return mRefreshEnabled && !ViewUtils.canChildScrollUp(mTargetView) && mHasHeaderView &&
    ...
    private boolean onCheckCanLoadMore() {
        return mLoadMoreEnabled && !ViewUtils.canChildScrollDown(mTargetView) && mHasFooterView
    ...
    RefreshCallback mRefreshCallback = new RefreshCallback() {
        ...
        public void onPrepare() {
            if (mHeaderView != null && mHeaderView instanceof SwipeTrigger && STATUS
                    .isStatusDefault(mStatus)) {
    ...
    LoadMoreCallback mLoadMoreCallback = new LoadMoreCallback() {
        ...
        @Override
        public void onPrepare() {
            if (mFooterView != null && mFooterView instanceof SwipeTrigger && STATUS
                    .isStatusDefault(mStatus)) {
