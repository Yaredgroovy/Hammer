package com.vzan.geetionlib.widget.refreshlayout.style.google;
...
public class CircleProgressView extends android.support.v7.widget.AppCompatImageView {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mBackGroundColor = a.getColor(
                R.styleable.CircleProgressView_gcp_background_color, DEFAULT_CIRCLE_BG_LIGHT);

        mProgressColor = a.getColor(
                R.styleable.CircleProgressView_gcp_progress_color, DEFAULT_CIRCLE_BG_LIGHT);
        mColors = new int[]{mProgressColor};

        mInnerRadius = a.getDimensionPixelOffset(
                R.styleable.CircleProgressView_gcp_inner_radius, -1);

        mProgressStokeWidth = a.getDimensionPixelOffset(
                R.styleable.CircleProgressView_gcp_progress_stoke_width, (int)
                        (STROKE_WIDTH_LARGE * density));
        mArrowWidth = a.getDimensionPixelOffset(
                R.styleable.CircleProgressView_gcp_arrow_width, -1);
        mArrowHeight = a.getDimensionPixelOffset(
                R.styleable.CircleProgressView_gcp_arrow_height, -1);
        mTextSize = a.getDimensionPixelOffset(
                R.styleable.CircleProgressView_gcp_progress_text_size, (int) (DEFAULT_TEXT_SIZE *
                        density));
        mTextColor = a.getColor(
                R.styleable.CircleProgressView_gcp_progress_text_color, Color.BLACK);

        mShowArrow = a.getBoolean(R.styleable.CircleProgressView_gcp_show_arrow, true);
        mCircleBackgroundEnabled = a.getBoolean(R.styleable
                .CircleProgressView_gcp_enable_circle_background, true);


        mProgress = a.getInt(R.styleable.CircleProgressView_gcp_progress, 0);
        mMax = a.getInt(R.styleable.CircleProgressView_gcp_max, 100);
        int textVisible = a.getInt(R.styleable.CircleProgressView_gcp_progress_text_visibility, 1);
        if (textVisible != 1) {
            mIfDrawText = true;
        }
    ...
    private class OvalShadow extends OvalShape {
        ...
        public void draw(Canvas canvas, Paint paint) {
            final int viewWidth = CircleProgressView.this.getWidth();
            final int viewHeight = CircleProgressView.this.getHeight();
